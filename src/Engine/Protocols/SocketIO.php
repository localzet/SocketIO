...
class SocketIO
{
    public static function input($http_buffer, $connection)
    {
        ...
        $connection->onClose = '\localzet\SocketIO\Engine\Protocols\SocketIO::emitClose';
        ...
        if (!empty($connection->onRequest)) {
            ...
            // POST
            if ('\localzet\SocketIO\Engine\Protocols\SocketIO::onData' !== $connection->onMessage) {
                $connection->onMessage = '\localzet\SocketIO\Engine\Protocols\SocketIO::onData';
            }
            if (!$raw_body) {
                return 0;
            }
